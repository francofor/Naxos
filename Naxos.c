...
int coordInit(coord a, int nwords)
  // It sets a = 0
  // Always the same number of cycles
  {
    int i;
...
int coordCopy(coord a,coord b, int nwords)
  // It sets a = b
  // Always the same number of cycles
  {
    int i;
...
int coordHalf(coord a,coord b, int nwords)
  // It sets a = b/2
  // Always the same number of cycles
  {
    int i;
...
int coordAddAndHalf(coord c,coord a,coord b,coord p,int nwords)
...
int coordDouble (coord a,coord b,coord p,int nwords)
  ...
  // Always the same number of cycles
{
  ...
  else                                       // in order to maintain the same number of cycles
  {
    r = 0;
    for (i=0;i<nwords;i++)
    {
...
int coordAdd(coord c,coord a,coord b,coord p,int nwords)
...
int coordSub(coord c,coord a,coord b,coord p,int nwords)
...
int coordMul(coord c,coord a,coord b,coord p,int nwords)
  // It calculates c = a * b mod p with a,b< p
  // Always the same number of cycles
  {
    int i,j;
    ...
    coordCopy(t1,a,nwords);             // Initialized t1 to a
    coordCopy(t2,a,nwords);             // t2 is used only to avoid side channel attacks
    coordInit(t3,nwords);               // Initialize t3 = 0
    ...
    for (i = 0;i<nwords;i++)            // word by word
    {
      for (j = 0;j<BITS64;j++)          // bit by bit of the word
      {
        ...
        {
          coordAdd(t3,t3,t1,p,nwords);  // c = c + t1 mod p
        }
        else                            // in order to maintain the same number of cycles
        {
    ...
    coordCopy(c,t3,nwords);             // c = t3
    return 0;
  }

int coordInvML(coord c,coord a,coord p,int nwords)
  // It calculates c = inv(a) mod p with a < p
  // In order to maintain the same number of cycles it calculates
  // c = a^(p-2) mod p with the Montgomery Ladder
  // It is 17% slower than coordInv
  {
    coord r0, r1, k;
    int i, n, b;
    coord f0, f1;                    // Needed to maintain the same number of cycles
    int order;                       // Needed to maintain the same number of cycles

    order = coordMaxBit(p,nwords);   // Needed to maintain the same number of cycles
    coordInit(r0,nwords);
    r0[0] = 2;                       // r0 = 2
    coordSub(k,p,r0,p,nwords);       // k = p-2
    n = coordMaxBit(k,nwords);       // Calculates n
    r0[0] = 1;                       // r0 = 1
    coordCopy(r1,a,nwords);          // r1 = a
    ...
    for (i=order-1;i>-1;i--)
    {
      ...
      else                           // to maintain the same number of cycles
      {
        if (b == 0)
        {
    ...
    coordCopy(c,r0,nwords);
    return 0;
  }

int cProjToAffine(pointA* aA,pointP* bP,coord p,int nwords)
...
int cAffineToProj(pointP* aP, pointA* bA,int nwords)
  // It converts point bA with Affine coordinates in point aP in Projective coordinates
  //   aP->pZ is set to 1.
  {

    coordCopy(aP->pX,bA->aX,nwords);    // aP->pX = bA->aX
    coordCopy(aP->pY,bA->aY,nwords);    // aP->pY = bA->aY
    coordInit(aP->pZ,nwords);           // aP->pZ = 0
    aP->pZ[0] = 1;                      // aP->pZ = 1

    return 0;
  }

int copyPointP(pointP* aP, pointP* bP,int nwords)
  // It sets aP = bP
  // Always the same number of cycles
  {

    coordCopy(aP->pX,bP->pX,nwords);    // aP->pX = bP->pX
    coordCopy(aP->pY,bP->pY,nwords);    // aP->pY = bP->pY
    coordCopy(aP->pZ,bP->pZ,nwords);    // aP->pY = bP->pY
...
int aIsOnCurve(pointA* aA,coord a,coord b,coord p,int nwords)
  ...
  // It returns 1 when it is verified
  {
    ...
    {
      return 0;                     // the equation is not verified
    }
  }

int doubleU(pointP* Q,pointP* R,pointP* P,coord a,coord p,int nwords)
  ...
  // Always the same number of cycles
  {
     ...
     coordCopy(t1,P->pX,nwords); // t1 = X1
     coordCopy(t2,P->pY,nwords); // t2 = Y1
     ...
     coordCopy(Q->pX,t3,nwords); // QX = M^2 - 2S
     coordCopy(Q->pY,t8,nwords); // QY = M * (S - X(2P)) - 8L
     coordCopy(Q->pZ,t6,nwords); // QZ = 2Y1
     coordCopy(R->pX,t1,nwords); // RX = 4X1 * E
     coordCopy(R->pY,t4,nwords); // RY = 8L
     coordCopy(R->pZ,t6,nwords); // RZ = 2Y1


    return 1;
  }

int zAddC(pointP* R,pointP* S,pointP* P,pointP* Q,coord p,int nwords)
  ...
  // It calculates R=P+Q and S=P-Q with input P and Q same Z and resulting R and S same Z3
  {
     ...
     coordCopy(t1,P->pX,nwords); // t1 = X1
     coordCopy(t2,P->pY,nwords); // t2 = Y1
     coordCopy(t3,P->pZ,nwords); // t3 = Z
     coordCopy(t4,Q->pX,nwords); // t4 = X2
     coordCopy(t5,Q->pY,nwords); // t5 = Y2
     ...
     coordCopy(R->pX,t1,nwords); // RX = t1
     coordCopy(R->pY,t2,nwords); // RY = t2
     coordCopy(R->pZ,t3,nwords); // RZ = t3
     coordCopy(S->pX,t4,nwords); // SX = t4
     coordCopy(S->pY,t5,nwords); // SY = t5
     coordCopy(S->pZ,t3,nwords); // SZ = t3

     return 1;
  }

int zAddU(pointP* R,pointP* P2,pointP* P,pointP* Q,coord p,int nwords)
  ...
  //  same Z1 and resulting R and P2 same Z3
  {
     ...
     coordCopy(R->pX,t4,nwords);  // RX  = t4
     coordCopy(R->pY,t5,nwords);  // RY  = t5
     coordCopy(R->pZ,t3,nwords);  // RZ  = t3
     coordCopy(P2->pX,t1,nwords); // P2X = t1
     coordCopy(P2->pY,t2,nwords); // P2Y = t2
     coordCopy(P2->pZ,t3,nwords); // P2Z = t3

    return 1;
  }

int scalarMult(pointA* Q,coord k,pointA* P,coord a,coord p,int nwords)
  ...
  // Output: Q = kP
  // To be adapted in order to get the same number of cycles
  //   it must be assured that n is always the same...
  {
     pointP R0,R1;
     pointP S0,S1;                          // Needed to maintain the same number of cycles
     int i, n, b;
     int order;                             // Needed to maintain the same number of cycles

     order = coordMaxBit(p,nwords);         // Needed to maintain the same number of cycles
     n = coordMaxBit(k,nwords);             // Calculates n
     coordCopy(R0.pX,P->aX,nwords);
     coordCopy(R0.pY,P->aY,nwords);         // R0=P
...
int byteToWord(coord arrayW,uint8_t *arrayB,int byteLen)
...
int wordToByte(uint8_t *arrayB,coord arrayW,int wordLen)
...
int  convPointToBytes(keyC pX,keyC pY,pointA* aP,ellipticCurve* curve)
// Convert pkA in byte arrays pkAx and pkAy
  {
...
//   pk = G*sk
  {
    ...
    byteToWord(t1,sk,curveN->bsize/8);         // Convert sk to t in coord format
...
// Calculate kA using the x coordinates of the points on the curve
...
//   -5 = internal error

  {
    ...
    wordToByte(msg,t1A.aX,curveN->wsize);                // Convert coord x of t1A in byte array format
    wordToByte(&msg[byteLen],t2A.aX,curveN->wsize);      // Convert coord x of t2A in byte array format and append to msg

    inputByteLen = byteLen*2;

    wordToByte(&msg[inputByteLen],t3A.aX,curveN->wsize); // Convert coord x of t3A in byte array format and append to msg
...
// Calculate kB using the x coordinates of the points on the curve
...
//   -5 = internal error

  {
    ...
    wordToByte(msg,t1B.aX,curveN->wsize);                // Convert coord x of t1B in byte array format
    wordToByte(&msg[byteLen],t2B.aX,curveN->wsize);      // Convert coord x of t2B in byte array format and append to msg

    inputByteLen = byteLen*2;

    wordToByte(&msg[inputByteLen],t3B.aX,curveN->wsize); // Convert coord x of t3B in byte array format and append to msg
